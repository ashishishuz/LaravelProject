            ...
            <form action="" name="searchForm" id="searchForm">
                <div class="card border-0 shadow p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <!-- search job on filters btn  -->
                         <button class="btn btn-primary" type="submit">Search</button>
                         <!-- Reset filter -->
                         <a href="{{route('jobs')}}" class="btn btn-secondary mt-3" type="submit">Reset</a>
                ...
                <div class="job_listing_area">                    
                    ...
                    <div class="row">
                        @if($jobs->isNotEmpty())
                            ...
                            <div class="card border-0 p-3 shadow mb-4">
                                <div class="card-body">
                                    ...
                                    <div class="bg-light p-3 border">
                                        ...
                                        @endif
                                        <!-- <p class="mb-0">
                                        <span class="ps-1"><i class="fas fa-tools"></i> Skills: {{ $job->keywords }}</span><br>
                                        <span class="ps-1"><i class="fas fa-briefcase"></i> Category: {{ $job->category->name }}</span><br>
                                        <span class="ps-1"><i class="fas fa-user-tie"></i> Experience: {{ $job->experience }} year</span>
                                        </p> -->
                                    </div>
                            ...
                            @endforeach
                        @else
                        <div class="col-md-12"> <i class="fas fa-search"></i>No Jobs to Display</div>
...
<script>
    $("#searchForm").submit(function(e){
        ...
        var checkedJobTypes= $("input:checkbox[name='job_type']:checked").map(function(){   //selecting checkboxes that are selected
        ...
        if(keyword != ""){
            url += '&keyword='+keyword; //adding location to url i.e concat
        }

         // if location has value
         if(location != ""){
            url += '&location='+location; //adding location to url i.e concat
        }

         // if category has value
         if(category != ""){
            url += '&category='+category; //adding location to url i.e concat
        }

        // if experience has value
        if(experience != ""){
            url += '&experience='+experience; //adding location to url i.e concat
        }

        // if jobtype checkedbox has value
